...
For an ordinary saved script, run `%run "relative/path.py"` in a notebook or IPython console. For browser applications, use the installed course helper, for example `from mansci_tools import run_app` followed by `run_app("party.py", kind="streamlit")`. It resolves the shared coding folder, starts from the project folder and provides the appropriate Jupyter proxy link. On the JupyterHub VM it also displays an authenticated phone link and QR code; the student signs into the VM on the phone if asked, and no credential is embedded in the code. Local installations remain loopback-only and do not display a phone QR code. For Flask it supplies the allocated port through the standard Flask command, so a conventional file ending in `app.run(debug=True)` does not need ManSci-specific changes. Use `stop_app("party.py")` to stop it. Terminal commands remain useful supplementary knowledge; change into the project folder first so relative files work.
