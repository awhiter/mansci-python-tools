...
Release 2026.09.21.5 is public for staff testing, not yet approved for student rollout.
...
For an ordinary saved script, run `%run "relative/path.py"` in a notebook or IPython console. For browser applications, use the installed course helper, for example `from mansci_tools import run_app, stop_app` followed by `run_app("party.py", kind="streamlit")`. It resolves the shared coding folder, starts from the project folder and provides the appropriate Jupyter proxy link. On the JupyterHub VM it also displays an authenticated classroom link and QR code. Anyone with an account on the same ManSci VM can sign in and interact with the running prototype; no credential is embedded, and the share expires when the app stops. Local installations remain loopback-only and do not display a phone QR code. For Flask the runner supplies the allocated port through the standard Flask command, so a conventional file ending in `app.run(debug=True)` does not need ManSci-specific changes. Because the example imports both functions, use `stop_app("party.py")` to stop it; otherwise use the self-contained command `from mansci_tools import stop_app; stop_app("party.py")`. Terminal commands remain useful supplementary knowledge; change into the project folder first so relative files work.
