"""A tool-free local coding assistant for ManSci Staff Lab."""
...
class QwenLocalChatPersona(BasePersona):
    """Use Qwen as a conventional chat model, without notebook agent tools."""
    ...
    def conversation_messages(self, current: Message) -> list[dict[str, str]]:
        """Return a bounded transcript containing this user and this persona."""
        ...
        # The chat normally contains the current message already. Include it
        # explicitly if a future frontend calls the persona before inserting it.
        ...
        for item in reversed(candidates[-self.MAX_HISTORY_MESSAGES :]):
            ...
            transcript.append(
                {
                    "role": "assistant" if item.sender == self.id else "user",
                    "content": body,
                }
            )
            remaining -= len(body)
