...
def wait_for_server(lab, runtime, workspace, stop):
    ...
    env.update(lab.server_environment())
...
def run():
    import staff_lab as lab
    owner = WindowOwner(lab.data_dir())
    workers = []
    try:
        if not owner.acquire():
            if owner.request_focus(): return 0
            raise RuntimeError('The existing ManSci Staff Lab window is not responding. Close it normally before retrying.')
        owner.listen()
        import webview
        webview.settings['ALLOW_DOWNLOADS'] = True
        window = webview.create_window('ManSci Staff Lab', width=1200, height=800,
            min_size=(700, 500), confirm_close=True, text_select=True, zoomable=True,
            html='<h2>Starting ManSci Staff Lab…</h2><p>Please wait while the local notebook server starts.</p>')
        ...
        def prepare():
            try:
                ...
                if not owner.stop.is_set():
                    message = '<h2>ManSci Staff Lab could not start</h2><p>' + html.escape(str(error)) + '</p><p>Close this window, resolve the error and relaunch.</p>'
                    window.load_html(message)
        def service():
