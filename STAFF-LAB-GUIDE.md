...
On the JupyterHub VM, `run_app()` also displays an authenticated classroom link and QR code. Anyone with an account on the same ManSci VM can scan it, sign in with their own account and interact with the presenter's running prototype. The code contains no credential and expires when the app stops. Staff Lab and other local installations remain loopback-only and therefore do not display a phone QR code.
